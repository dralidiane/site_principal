...
import laserCo2UltrapulsadoImg from '../assets/logos/LaserCO₂-Ultrapulsado.webp';
...
const advancedProcedures = [
  {
    ...
    ctaId: 'laser-co2',
    image: laserCo2Img,
    imageAlt: laserCo2UltrapulsadoImg // Segunda imagem para flip
  },
  {
    icon: Syringe,
    title: 'Preenchimento Labial',
    subtitle: 'Lábios Volumosos e Naturais',
    benefit: 'Aumenta o volume, define o contorno e hidrata os lábios com ácido hialurônico de alta qualidade.',
    transformation: 'Lábios mais cheios, simétricos e com aspecto jovem e natural.',
    cta: 'Quero lábios perfeitos',
    ctaId: 'preenchimento-labial',
    image: preenchimentoLabialImg // Imagem do Preenchimento Labial
  },
  {
    icon: Cpu,
  ...
  },
  {
    ...
    ctaId: 'fios-pdo',
    image: fiosPdoImg // Imagem dos Fios de PDO
  }
];
          ...
          {advancedProcedures.map((procedure, index) => (
            ...
            >
              ...
              <div className={`relative ${index % 2 === 1 ? 'lg:col-start-1 lg:row-start-1' : ''} order-2 lg:order-none`}>
                <div className={`aspect-[4/5] ${procedure.ctaId === 'radiofrequencia' || procedure.ctaId === 'laser-co2' ? 'bg-white' : 'bg-gradient-to-br from-[#f3e1db] to-[#d9b6af]'} rounded-sm ${procedure.ctaId === 'radiofrequencia' || procedure.ctaId === 'laser-co2' ? '' : 'shadow-xl'} overflow-hidden`}>
                  {procedure.image ? (
                    procedure.imageAlt ? (
                      // Card flip container for procedures with two images
                      <>
                        <style>{`
                          @keyframes flip {
                            0%, 45% {
                          ...
                          .flip-card-${index} .flip-card-front,
                          .flip-card-${index} .flip-card-back {
                            position: absolute;
                            width: 100%;
                            height: 100%;
                        ...
                        <div className={`flip-card-${index}`}>
                          ...
                          <div className="flip-card-back">
                            <img
                              src={procedure.imageAlt}
                              alt={`Resultado alternativo do procedimento ${procedure.title}`}
                      ...
                      // Regular image for procedures without imageAlt
                      <img
                        src={procedure.image}
                        alt={`Resultado do procedimento ${procedure.title}`}
                        className={`w-full h-full ${procedure.ctaId === 'radiofrequencia' ? 'object-contain' : 'object-cover'}`}
